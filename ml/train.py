...
def main():
    ...
    if problem == "burgers_1d":
        ...
        if not os.path.isfile(dataname):
            ...
            return
        train_loader, test_loader, N_i, N_o, _ = load_burgers_1d(
        ...
        for epoch in range(1, cfg.num_epochs + 1):
            lr = get_lr(epoch, cfg.lr_schedule)
            ...
            with torch.no_grad():
                for inputs, targets in test_loader:
                    epoch_test += criterion(model(inputs), targets).item()
            test_loss_history.append(epoch_test / len(test_loader))
            print(f"Epoch [{epoch}/{cfg.num_epochs}], lr={lr:.0e}, Train: {train_loss_history[-1]:.10f}, Test: {test_loss_history[-1]:.10f}")
        save_checkpoint(
            model, optimizer, cfg.num_epochs, train_loss_history, test_loss_history,
            os.path.join(data_dir, cfg.model_pth),
            hidden_size=cfg.hidden_size, num_hidden_layers=cfg.num_hidden_layers,
        )
        savemat(os.path.join(data_dir, cfg.error_mat), {"train_err": train_loss_history, "test_err": test_loss_history})
    ...
    if problem == "wave_2d_linear":
        ...
        if not os.path.isfile(dataname):
            ...
            return
        train_loader, test_loader, N_i, N_o, _ = load_wave_2d_linear(
            dataname, device, b_size=cfg.b_size, test_split=0.2
        )
        n_out = N_o // 2
        ...
        for epoch in range(1, cfg.num_epochs + 1):
            ...
            for inputs, targets in train_loader:
                outputs = model(inputs)
                loss = w_u * criterion(outputs[:, :n_out], targets[:, :n_out]) + w_v * criterion(outputs[:, n_out:], targets[:, n_out:])
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                epoch_train += loss.item()
            ...
            with torch.no_grad():
                for inputs, targets in test_loader:
                    loss = w_u * criterion(model(inputs)[:, :n_out], targets[:, :n_out]) + w_v * criterion(model(inputs)[:, n_out:], targets[:, n_out:])
                    epoch_test += loss.item()
            test_loss_history.append(epoch_test / len(test_loader))
            print(f"Epoch [{epoch}/{cfg.num_epochs}], lr={lr:.0e}, Train: {train_loss_history[-1]:.10f}, Test: {test_loss_history[-1]:.10f}")
        save_checkpoint(
            model, optimizer, cfg.num_epochs, train_loss_history, test_loss_history,
        ...
        for epoch in range(1, cfg.num_epochs + 1):
            ...
            with torch.no_grad():
                for inputs, targets in test_loader:
                    ...
                    data_loss_vec = diff.mean(dim=(0, 2, 3))
                    tv_vec = tv_isotropic_per_channel(outputs)
                    epoch_test += (data_loss_vec * component_weights + lam_tv * tv_vec).sum().item()
            test_loss_history.append(epoch_test / len(test_loader))
            print(f"Epoch [{epoch}/{cfg.num_epochs}], Train: {train_loss_history[-1]:.10f}, Test: {test_loss_history[-1]:.10f}", flush=True)
        save_checkpoint(
            model, optimizer, cfg.num_epochs, train_loss_history, test_loss_history,
            os.path.join(data_dir, cfg.model_pth),
            base=cfg.base,
