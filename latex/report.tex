...
\begin{itemize}
  ...
  \item \textbf{2D nonlinear shallow water:} state $(h,q_x,q_y)$ solved by a spectral reference model. NN: UNet patch-to-patch predictor with residual learning, trained with per-channel absolute TV regularization.
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    Steps per saved frame $T_{\mathrm{screen}}$ & 10 \\
    Patch half-width $n_{\mathrm{st}}$ & 32 \\
    Interior width $n_{\mathrm{wd}}$ & 32 \\
    Viscosity $\nu_h$ & 0 \\
    ...
    Epochs & 150 \\
    Model type & UNet \\
    UNet base channels & 32 \\
    Pooling & avg \\
    Residual prediction & False \\
    TV smooth weight & [0, 0.1, 0.1] \\
    TV smooth mode & absolute \\
    Data loss channel weights & [1.0, 1.0, 1.0] \\
    \bottomrule
  \end{tabular}
\end{table}
...
Comparison at 6 output times over $t\in[0,2]$ with \texttt{compare\_t\_end=2.0}. Runtime for this run: FV total $2.778$\,s, NN total $0.5852$\,s, speedup $4.7\times$.
...
Comparison at 6 aligned frames with \texttt{compare\_TF=2.0} and \texttt{compare\_ic=ring}. Runtime for this run: spectral $10.323$\,s, NN $0.4576$\,s, speedup $22.6\times$.
...
Comparison at 8 aligned frames for $(h-h_0,q_x,q_y)$ with \texttt{compare\_TF=8.0} and \texttt{compare\_ic=random}. Both rollouts start after warmup time $T=2.0$. Final rollout metrics for this run: mean L1 error $0.047030$, max-frame L1 error $0.075241$, spectral runtime $161.058$\,s, NN runtime $10.251$\,s, speedup $15.7\times$.
