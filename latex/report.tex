...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    Domain $L_x=L_y$ & $2\pi$ \\
    $g$, $h_0$ & 9.8, 1.0 \\
    Steps per saved frame $T_{\mathrm{screen}}$ & 20 \\
    Patch half-width $n_{\mathrm{st}}$ & 24 \\
    Interior width $n_{\mathrm{wd}}$ & 32 \\
    Viscosity $\nu_h$ & 0 \\
...
Comparison at 8 aligned frames for $(h-h_0,q_x,q_y)$ with \texttt{compare\_TF=3.0} and \texttt{compare\_ic=random}. Both rollouts start after warmup time $T=6.0$. Final rollout metrics for this run: mean L1 error $0.008592$, max-frame L1 error $0.015684$, spectral runtime $108.152$\,s, NN runtime $5.6242$\,s, speedup $19.23\times$.
