...
class myNN(nn.Module):
    def __init__(
        ...
        self.outlay = nn.Linear(hidden_size, output_size)

    def forward(self, x):
        for layer in self.hidden:
            x = self.activation(layer(x))
        return self.outlay(x)


def mat2pyt(filename, device, b_size=100, test_split=0.2):
    """Load .mat file from path; filename can be full path or 'data_res.mat'."""
    data = scipy.io.loadmat(filename)
    data_in = data["input_tensor"]   # N_input x N_samples
    data_out = data["output_tensor"]  # N_output x N_samples
